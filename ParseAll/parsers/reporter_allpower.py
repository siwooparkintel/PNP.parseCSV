...
def flatten_data(entry, socwatch_targets, picks):
...
def reportAllPowerAndType2(result_path, hobl_data, socwatch_targets, picks) :
    df = pd.DataFrame([flatten_data(entry, socwatch_targets, picks) for entry in hobl_data])
