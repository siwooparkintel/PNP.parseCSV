...
def coreFreqResidencyTable(table, core_type_dict):
    ...
    # print("[header_start] ", header_start, combined_data)
    ...
    for index in range(1, len(copied), 1) :  # this is excluding freq 0, idle. : for index in range(1, len(copied)-1, 1) :
        ...
        if key == "0" : 
            key = "0-idle"
        # "LNC4601-4700" is expected 
        for cell_idx in range(len(header)) :
            # print("[coreFreqResidencyTable] ", cell_idx, len(line_data_only), line_data_only)
        ...
        # sum_list = ["-", "-"], the list size will be the number of core type, LNC, SKT, two in LNL case
    ...
    for idx, core_name in enumerate(combined_data[header_start]):
        for pstate in combined_data:
            # print(core_name, idx, pstate, combined_data[pstate], str(core_name)+" "+str(pstate), type(combined_data[pstate]))
